...
public class StatisticManager extends AbstractFeedbackManager implements
    ...
    protected void submitStatisticFile(final File file) {
         ...
         * 
         * @Override public IStatus run(IProgressMonitor monitor) { try {
         * FileSubmitter.uploadStatisticFile(file, monitor); } catch
         * (IOException e) { String msg =
         * String.format("Couldn't upload file: %s. %s", e.getMessage(),
         * e.getCause() != null ? e.getCause() .getMessage() : "");
         * log.error(msg); return new CancelableProcess.Status(IStatus.ERROR,
         * Saros.SAROS, msg, e); } catch (OperationCanceledException e) { return
         * CancelableProcess.Status.CANCEL_STATUS; } finally { monitor.done(); }
