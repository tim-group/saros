...
import de.fu_berlin.inf.dpp.intellij.editor.mock.internal.*;
...
/**
 * The EditorManagerEcl is responsible for handling all editors in a DPP-session.
 ...
 * <p/>
 * The EditorManagerEcl contains the testable logic. All untestable logic should
...
public class EditorManagerEcl extends AbstractActivityProducerAndConsumer implements IEditorManager
{
    ...
    protected static final Logger log = Logger.getLogger(EditorManagerEcl.class
            .getName());

    protected SharedEditorListenerDispatch editorListenerDispatch = new SharedEditorListenerDispatch();

    protected EditorAPIEcl editorAPI;
    ...
    private ISarosSessionListener sessionListener = new AbstractSarosSessionListener()
    {
        ...
        public void sessionStarted(ISarosSession newSarosSession)
        {
            ...
            sarosSession.addActivityProducerAndConsumer(EditorManagerEcl.this);
        ...
        public void projectAdded(String projectID)
        {
            SWTUtils.runSafeSWTSync(log, new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    Set<IEditorPart> allOpenEditorParts = EditorAPIEcl
                            .getOpenEditors();
     ...
     * @Inject
     */
    public EditorManagerEcl(ISarosSessionManager sessionManager,
            EditorAPIEcl editorApi, IPreferenceStore preferenceStore)
    {

        System.out.println("EditorManagerEcl.EditorManagerEcl");

        log.trace("EditorManagerEcl initialized");
    ...
    {
        System.out.println("EditorManagerEcl.execEditorActivity>"+editorActivity);
    ...
    protected void execTextEdit(TextEditActivity textEdit)
    {
        ...
        log.trace("EditorManagerEcl.execTextEdit invoked>"+textEdit);
    ...
    protected void execTextSelection(TextSelectionActivity selection)
    {
        ...
        log.trace("EditorManagerEcl.execTextSelection invoked");

        SPath path = selection.getPath();

        if (path == null)
        {
            EditorManagerEcl.log
        ...
        Set<IEditorPart> editors = EditorManagerEcl.this.editorPool
                .getEditors(path);
    ...
    protected void execViewport(ViewportActivity viewport)
    {
        ...
        log.trace("EditorManagerEcl.execViewport invoked>"+viewport);
    ...
    protected void execActivated(User user, SPath path)
    {
        ...
        log.trace("EditorManagerEcl.execActivated invoked");
    ...
    protected void execClosed(User user, SPath path)
    {
        ...
        log.trace("EditorManagerEcl.execClosed invoked>"+path);
    ...
    public void partClosed(IEditorPart editorPart)
    {
        ...
        log.trace("EditorManagerEcl.partClosed invoked");
    ...
    protected boolean isSharedEditor(IEditorPart editorPart)
    {
        ...
        if (EditorAPIEcl.getViewer(editorPart) == null)
        {
            return false;
        }
     ...
     * @param offset       The position into the document of the given file, where the
     *                     change started.
     * @param replacedText The text which is to be replaced by this operation at the
     *                     given offset (is "" if this operation is only inserting text)
     * @param text         The text which is to be inserted at the given offset instead
    ...
    public void saveLazy(final SPath path) throws FileNotFoundException
    {
        ...
        try
        {
            SWTUtils.runSWTSync1(new Callable<Object>()
            {
                @Override
     ...
     *
     * @param path the project relative path to the file that is supposed to be
     *             saved to disk.
    ...
    public void saveText(SPath path)
    {
        ...
        log.trace("EditorManagerEcl.saveText (" + file.getName() + ") invoked");
        ...
        try
        {
            ...
            log.trace("EditorManagerEcl.saveText Annotations on the IDocument "
                    + "are set");
            ...
            log.trace("EditorManagerEcl.saveText File " + file.getName()
                    + " will be reseted");
     ...
     *
     * @param path the project relative path to the resource that the user with
     *             {@link Permission#WRITE_ACCESS} was editing.
     */
    public void sendEditorActivitySaved(SPath path)
    {

        log.trace("EditorManagerEcl.sendEditorActivitySaved started for "
                + path.toString());
    ...
    public void refreshAnnotations(IEditorPart editorPart)
    {
        ...
        for (User user : sarosSession.getUsers())
        {
            ...
            RemoteEditorManager.RemoteEditorState remoteEditorState = remoteEditorManager
                    .getEditorState(user);
            ...
            RemoteEditorManager.RemoteEditor remoteEditor = remoteEditorState.getRemoteEditor(path);
     ...
     * editor.
     *
     * @param path Path of the file to check if it is opened.
     * @return <ul>
     ...
     *
     * @param path Path of the Editor to close.
     */
    public void closeEditor(SPath path)
    {
        ...
        for (IEditorPart iEditorPart : EditorAPIEcl.getOpenEditors())
        {
     ...
     * @param selection    text selection of the followed user
     */
            ...
            ITextSelection selection)
    {
        ...
        ILineRange localViewport = EditorAPIEcl.getViewport(viewer);
